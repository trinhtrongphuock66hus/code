...
void pillsTakingSetting(Alarm &alarmEEPROM, DateTime &alarm, bool &settingMode) {
  ...
  if (order == 3) {
    if (millis() % 1000 < 500) {
      display.print("Pills: ");
      display.print(alarmEEPROM.pills);
    } else {
      display.print("Pills: ");
      display.print(' ');
    }
...
void printDateAlarm(DateTime dt, int i) {
  display.setFont(&FreeMono9pt7b);
  display.setTextSize(0);
  display.setCursor(0, 40);

  if (i == 1) {
    // Nếu i bằng 1, thì hiển thị giờ với hiệu ứng nhấp nháy
    ...
    // Nếu i bằng 2, thì hiển thị phút với hiệu ứng nhấp nháy
    display.print(dt.hour());
    display.print(':');
    display.setCursor(30, 40); // Đặt vị trí cho hiển thị phút
    if (millis() % 1000 < 500) {
      display.print(dt.minute());
      display.print(' ');
    } else {
